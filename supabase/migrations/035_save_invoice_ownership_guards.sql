-- =========================================================================
-- Migration 035: save_invoice tenant ownership guards
--
-- Scope note: POS save_invoice is called by authenticated shop staff. The
-- shop-level tenant guard (assert_authenticated_shop, migration 032) already
-- blocks writing under another shop's id. This migration closes two narrower
-- integrity gaps that the shop guard alone does not cover:
--
--   1. Per-item product ownership - a tampered payload could reference a
--      product_id from ANOTHER shop. We now reject any non-null product_id
--      that does not belong to the caller's shop. NULL product_ids remain
--      valid (manual / non-catalog line items).
--   2. Credit (khata) customer ownership - credit_ledger previously inserted
--      a caller-supplied customer_id without verifying it belongs to the
--      shop. We now reject a credit sale whose customer_id is foreign.
--
-- NOT in scope (deliberate, see ship decision): POS totals/GST are still
-- taken from the client. That is a separate server-side-recompute effort and
-- the POS threat model is own-shop staff, not cross-tenant theft.
--
-- This restates the full save_invoice definition (incorporating the 032
-- assert_authenticated_shop guard) so the new checks are unambiguous rather
-- than string-patched in.
-- =========================================================================
...
BEGIN
  ...
  -- == Tenant guard: caller must be active staff of this shop (migration 032) ==
  ...
  -- Any invoice customer reference must belong to this shop.
  IF v_customer_id IS NOT NULL THEN
    SELECT true INTO v_customer_ok
    FROM customers
    WHERE id = v_customer_id
      AND shop_id = v_shop_id;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'Customer % does not belong to shop %', v_customer_id, v_shop_id;
    END IF;
  END IF;
  ...
  LOOP
    ...
    -- == Per-item ownership guard ==
    -- Non-null product_id MUST belong to this shop. NULL product_id is a
    -- valid manual / non-catalog line item and skips the catalog checks.
    IF v_product_id IS NOT NULL THEN
      SELECT is_stock_tracked
      INTO v_is_tracked
      FROM products
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Product % does not belong to shop %', v_product_id, v_shop_id;
      END IF;
    ELSE
      v_is_tracked := false;
    END IF;
    ...
    -- POS allows stock to go negative by design (speed-billing / offline
    -- tolerance); no exception is raised on shortfall.
  ...
  IF p_loyalty_entry IS NOT NULL AND p_loyalty_entry->>'customer_id' IS NOT NULL THEN
    v_loyalty_customer_id := (p_loyalty_entry->>'customer_id')::uuid;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Loyalty customer % does not belong to shop %', v_loyalty_customer_id, v_shop_id;
    END IF;
  ...
  -- =======================================================================
  -- 4. KHATA (CREDIT) ENGINE - validate same-shop customer before ledger write
  -- =======================================================================
  IF v_payment_mode = 'credit' AND v_customer_id IS NOT NULL AND v_total_paise > 0 THEN
    -- == Credit customer ownership guard ==
    SELECT true INTO v_customer_ok
    FROM customers
    WHERE id = v_customer_id
      AND shop_id = v_shop_id;
