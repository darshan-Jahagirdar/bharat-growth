-- =========================================================================
-- Migration 039: Storefront marketing consent
--
-- Restates create_online_order (from migration 034, the latest definition)
-- with an OPTIONAL marketing_consent flag in p_order:
--   * absent/false  -> no change to the customer's marketing consent
--     (an unticked box is "no answer", not a revocation — only the
--     opt-out webhook or an erasure flow revokes)
--   * true          -> dpdp_marketing_consent set with OR semantics on the
--     upsert (never downgrades) + an auditable consent_logs row
--
-- Fully backward compatible: existing checkout payloads without the key
...
BEGIN
  ...
  v_marketing_consent := COALESCE((p_order->>'marketing_consent')::boolean, false);
  ...
  INSERT INTO customers (
    ...
    dpdp_data_consent, dpdp_marketing_consent, consent_collected_at
  ) VALUES (
    v_shop_id, v_customer_phone, v_customer_name, v_delivery_address, 'new',
    0, 0, 0,
    true, v_marketing_consent, now()
  )
  ...
  SET name = CASE
      ...
      -- OR semantics: an unticked box never downgrades an existing opt-in
      dpdp_marketing_consent = customers.dpdp_marketing_consent OR EXCLUDED.dpdp_marketing_consent,
  ...
  IF v_marketing_consent THEN
    INSERT INTO consent_logs (
      ...
      v_shop_id, v_customer_id, 'whatsapp_marketing', 'granted', 'in_app',
      v_ip, v_user_agent,
      jsonb_build_object('source', 'storefront_checkout', 'order_channel', 'online_storefront')
    );
  END IF;
